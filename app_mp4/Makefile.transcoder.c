...
INSTALL = install
CC = gcc 
INCLUDE += -I$(FFMPEGDIR) -I$(FFMPEGDIR)/libavcodec -I$(FFMPEGDIR)/libswscale
INCLUDE += -I~/mp4v2-src/include
LIBS = -L$(FFMPEGDIR)/libavcodec -L$(FFMPEGDIR)/libavutil -lavcodec -lavutil -lswscale
LIBS += ~/mp4v2-src/.libs/libmp4v2.a -lstdc++

DEBUG := -g 

CFLAGS =  -fPIC -DAST_MODULE=\"app_mp4\"   $(DEBUG) -D_REENTRANT -D_GNU_SOURCE -DVIDEOCAPS
...
app_mp4.so : $(OBJS)
	$(CC) -pg -shared -Xlinker -x -o $@ $(OBJS) $(LIBS) $(RPATH) -lrt

 ...
 *		Emmanuel BUU			emmanuel.buu@ives.fr
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
  ...
  if (doCreate || doHint) {
	...
	if ((!strcmp(extension, ".amr")) || (!strcmp(extension, ".263"))) {
		mp4File = MP4CreateEx(mp4FileName,
				      ...
				      1,  // add ftyp atom
				      0,  // don't add iods
				      (char *) p3gppSupportedBrands[0],
				      0x0001,
				      (char**) p3gppSupportedBrands,
				      sizeof(p3gppSupportedBrands) / sizeof(p3gppSupportedBrands[0]));
	} else {
	  mp4File = MP4Create(mp4FileName, createFlags);
	}
	if (mp4File) {
    ...
    if (retval) 
    {
      ...
      sprintf(buffer, "mp4creator %s", MP4V2_PROJECT_version);
    ...
    MP4FileHandle outputFile = MP4CreateEx(outputFileName, createFlags, true, true);
